...
class Semantics:
    ...
    def store_global_statements(self):
        ...
        self.var_tables['global'] = self.program_vars['program'].store_global_vars('program')
    ...
    def handle_class_extension(self, id_extended_class):
        if id_extended_class not in self.class_dir:
            raise KeyError("Can not extend from non existent class: " + id_extended_class)
        elif self.class_dir[id_extended_class]['visibility'] == 'private':
            raise KeyError("Can not extend from private class: " + id_extended_class)
        else:
    ...
    def end_main(self):
        self.var_tables['main'] = self.current_func_dir.delete_var_table('main')
        self.code_gen.reset_t_counter()

    def init_main(self):
        self.current_func_dir.add_function('void', 'main', self.code_gen.counter)
        self.code_gen.current_scope = 'local'
    ...
    def process_void_module_declaration(self, id):
        if self.code_gen.current_scope != 'interfaces':
            self.current_func_dir.add_function('void', id, self.code_gen.counter)
            self.current_table = self.current_func_dir.get_var_table()
        else:
            self.current_interface.add_function('void', id, self.code_gen.counter)
    ...
    def process_module_ret_declaration(self, type_atomic, id):
        if self.code_gen.current_scope != 'interfaces':
            self.current_func_dir.add_function(type_atomic, id, self.code_gen.counter)
            ...
            self.current_interface.add_function(type_atomic, id, self.code_gen.counter)
            self.current_interface.params = {}
    ...
    def process_object_call(self, id, address):
        if self.current_function:
            ...
            if ret_type != "void":
                ...
                result = self.code_gen.call_return(str(address) + '.' + str(mem_dir), ret_type) #Doubt Comment Luis
    ...
    def validate_object_call(self, obj, element): #Comment Luis
        ...
        if element in f_list:
            ...
            self.current_function_types = list(f_list[element]['params'].values())
    ...
    def validate_function_existance(self, function_id):
        if function_id in self.current_func_dir.directory:
            ...
            self.current_function_ret = self.current_func_dir.directory[self.current_function]['return_type']
        else:
            raise NameError("Function call to undefined function: " + function_id)
    
    def process_declaration(self):
        tp = self.current_table.type_var
        self.current_table.register(self.code_gen.current_scope, self.cte_table)
        for obj in self.object_queue:
            self.code_gen.add_object(tp, self.current_table.table[obj]['virtual_address'])
        self.object_queue = []
    ...
    def process_initialized_declaration(self, id, operator):
        ...
        self.code_gen.addOperand(self.current_table.table[id]['virtual_address'])
    ...
    def set_array_dims(self):
        if self.current_arr:
            self.code_gen.set_dim(self.current_arr['dims'], self.cte_table.insert_cte(0, 'int'))
        else:
        ...
        if id not in self.current_table.table and self.current_table != self.var_table and self.current_table != self.global_var_table:
            if id not in self.var_table.table:
                if self.global_var_table.get_is_array(id):
                    ...
                    self.code_gen.types.push(self.global_var_table.get_type(id))
    ...
    def array_declaration(self, id):
        if self.current_arr and id == self.current_arr_id:
            cte_mem = self.cte_table.insert_cte(self.current_arr['virtual_address'], 'int')
            self.code_gen.final_arr(cte_mem, self.current_arr['type'], self.current_arr['dims'])
            self.current_arr = None
        ...
        for key in self.class_dir:
            for fn in self.class_dir[key]['functions']:
                rc = " " + self.class_dir[key]['functions'][fn]['return_type'] + \
                    " " + str(self.class_dir[key]['functions'][fn]['position']) + " "
